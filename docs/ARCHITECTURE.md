...
| Auth | Supabase Auth または Auth.js(Postgres連携) | 認証自体はメール/パスワードのみで開始できる。診断時の医院代表電話番号とは分離する |
...
| メール送信 | Resend等のトランザクションメールAPI(最終選定は未確定) | 診断結果をメールで届ける通知フローの要 |
...
[スコアリングエンジン: 6領域100点 + 敗因候補 + 改善TOP3を算出]
        |
        v
[diagnosis結果と医院代表電話番号を保存、メールで結果を通知]
        |
        v
...
1. 医院名、公式サイトURL、メールアドレス、医院代表電話番号(必須)を入力するフォーム
